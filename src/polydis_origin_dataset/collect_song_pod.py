from .dataset_loaders_pod import MusicDataLoaders
from .dataset_pod import SEED
import numpy as np
import torch
...
class SongDataset:
    def __init__(self, dataset):
        ...
        song_ind, song_len = get_valid_song_inds(dataset.valid_inds, min_bars=16)
    ...
    def get_song_batch(self, song_id, length=None, shift=0):
        ...
        batch = get_whole_song_data(
            self.dataset, self.song_ind[song_id], length + shift, shift
        )
        return batch
...
class SongDatasets:
    ...
    def get_msg(self, dataset_id, song_id, length, shift):
        ...
        return "_".join([str(dataset_id), str(song_id), str(length), str(shift)])
