...
def pr_to_onehot_pr(pr):
    ...
    sustain_data = pr[:, :] == 1
    silence_data = pr[:, :] == 0
    pr = np.stack([onset_data, sustain_data, silence_data], axis=-1).astype(np.int64)
    return pr
...
def target_to_3dtarget(
    ...
    for t, p in zip(*np.where(pr_mat != 0)):
        ...
        pr_mat3d[t, cur_idx[t],
                 1 : 6] = np.fromstring(" ".join(list(binary)), dtype=int, sep=" ")
        cur_idx[t] += 1
