...
import pandas as pd
import pickle
from .score_pod import PolyphonicMusic
from torch.utils.data import DataLoader
from .converter_pod import (
    ext_nmat_to_pr,
    ext_nmat_to_mel_pr,
    augment_pr,
    augment_mel_pr,
...
DATA_PATH = "./data/polydis/data/POP09-PIANOROLL-4-bin-quantization"
INDEX_FILE_PATH = "./data/polydis/data/index.xlsx"
SEED = 3345
...
class ArrangementDataset(Dataset):
    ...
    def __getitem__(self, id):
        ...
        acc_segments = [
            ext_nmat_to_pr(self._combine_segments(acc[i : i + 2]))
            for i in range(0, self.num_bar, 2)
        ]
...
def detrend_pianotree(piano_tree, c):
    ...
    chroma = np.array(
        [np.roll(cc, shift=-rr, axis=-1) for cc, rr in zip(c[:, 12 : 24], root)]
    )
    ...
    for t in range(8):
        ...
        for i in range(4):
            for j in range(16):
                is_note, is_bass, octave, scale_deg, n_state = convert_note(
                    pitch[t, i, j], chroma_state, rr, bb, deg_table, semi_table
                )
                if has_bass:
                    is_bass = 0
                else:
    ...
    notes = np.concatenate([is_notes, is_basses, octaves, degs, n_states, dur], axis=-1)
...
def get_chroma_state(chroma, map_dic):
    ...
    chroma_states[:, 1] = np.array(
        [map_dic[tuple(cc)] for cc in chroma[:, [1, 2]]], dtype=int
    )
    chroma_states[:, 2] = np.array(
        [map_dic[tuple(cc)] for cc in chroma[:, [3, 4]]], dtype=int
    )
    chroma_states[:, 3] = np.array(
        [map_dic[tuple(cc)] for cc in chroma[:, [5, 6]]], dtype=int
    )
    chroma_states[:, 5] = np.array(
        [map_dic[tuple(cc)] for cc in chroma[:, [8, 9]]], dtype=int
    )
    chroma_states[:, 6] = np.array(
        [map_dic[tuple(cc)] for cc in chroma[:, [10, 11]]], dtype=int
    )
...
def collect_data_fns():
    ...
    with open("data/ind.pkl", "wb") as dump_file:
        print("Data files dumped into ind.pkl.")
        pickle.dump(valid_files, dump_file)
    return valid_files


def init_music(fn):
    """
    get music data ready
    """
    data = np.load(fn)
    beat = data["beat"]
    chord = data["chord"]
...
def split_dataset(length, portion):
    train_ind = np.random.choice(
        length, int(length * portion / (portion + 1)), replace=False
    )
    valid_ind = np.setdiff1d(np.arange(0, length), train_ind)
    return train_ind, valid_ind


def wrap_dataset(fns, ids, shift_low, shift_high, num_bar=8, contain_chord=False):
    """
    shift_low, shift_high: [-6, 5], data augmentation
    """
    data = []
    indicator = []
    for i, ind in enumerate(ids):
        fn = fns[ind]
        music = init_music(fn)  # PolyphonicMusic
    ...
    dataset = ArrangementDataset(
        data, indicator, shift_low, shift_high, num_bar=num_bar, contain_chord=True
    )
    return dataset
...
def prepare_dataset(
    ...
    val_set = wrap_dataset(
        fns, val_ids, 0, 0, num_bar=num_bar, contain_chord=contain_chord
    )
