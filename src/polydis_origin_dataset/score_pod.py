from .converter_pod import ext_nmat_to_nmat, nmat_to_notes
...
class PolyphonicMusic:
    ...
    def break_tracks_to_bars(self, db_pos=None, db_ts=None):
        if db_pos is None or db_ts is None:
            db_pos, db_ts = self.beat_track.get_downbeats()
        broken_tracks = [
            self._break_track_to_bars(track, db_pos, db_ts) for track in self.tracks
        ]
    ...
    def prepare_data(self, num_bar=8, ts=4, mel_id=(0, ), acc_id=(1, 2)):
        ...
        def merge(tracks, ids):
            to_merge = [
                track for i, track in enumerate(tracks) if i in ids and len(track) > 0
            ]
    ...
    def regularize_chord_table(self):
        pre_cat = np.zeros(
            (self.beat_track.translation, self.chord_table.shape[1]),
            dtype=self.chord_table.dtype,
        )
        post_cat = np.zeros(
            (self.beat_track.post_translation, self.chord_table.shape[1]),
            dtype=self.chord_table.dtype,
        )
        self.chord_table = np.concatenate([pre_cat, self.chord_table, post_cat], axis=0)
    ...
    def convert_track_to_notes(
        self, track_ind=None, track_name=None, start=0.0, bpm=None
    ):
        if bpm is None:
            bpm = self.bpm
    ...
    def convert_tracks(self, track_ids=None, track_names=None, start=0.0, bpm=None):
        if bpm is None:
            bpm = self.bpm
        ...
        for track_id, track_name in zip(track_ids, track_names):
            notes = self.convert_track_to_notes(track_id, track_name, start, bpm)
            track_notes.append(notes)
        return track_notes

    def export_to_pretty_midi(
        self, track_ids=None, track_names=None, start=0.0, bpm=None
    ):
        if bpm is None:
            bpm = self.bpm
        if track_ids is None and track_names is None:
            ...
            track_ids = [self.track_name_list.index(name) for name in track_names]
    ...
    def write_midi(self, fn, track_ids=None, track_names=None, start=0.0, bpm=None):
        midi = self.export_to_pretty_midi(track_ids, track_names, start, bpm)
        midi.write(fn)
...
class BeatTrack:
    ...
    def _is_regularized_track(self):
        return self._is_pre_regularized_track() and self._is_post_regularized_track()
    ...
    def get_time_signature_change(self):
        ...
        ts_change_pos = np.concatenate(
            [
                np.zeros(1, dtype=int),
                np.where(beat_info[1 :] != np.roll(beat_info, 1)[1 :])[0] + 1,
            ]
        )
