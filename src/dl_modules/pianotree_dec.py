...
class PianoTreeDecoder(nn.Module):
    def __init__(
        ...
        self.dec_time_gru = nn.GRU(
            dec_z_in_size + 2 * dec_emb_hid_size + feat_emb_dim,
            dec_time_hid_size,
            num_layers=1,
            batch_first=True,
            bidirectional=False,
    ...
    def decode_note(self, note_summary, batch_size):
        ...
        est_durs = self.dur_out_linear(note_summary).reshape(
            batch_size, self.dur_width, 2
        )
    ...
    def recon_loss(
        ...
        pitch_loss_func = nn.CrossEntropyLoss(
            ignore_index=self.pitch_pad, reduction=reduction
        )
        ...
        dur_loss_func = nn.CrossEntropyLoss(
            ignore_index=self.dur_pad, reduction=reduction
        )
        if not weighted_dur:
    ...
    def grid_to_pr_and_notes(self, grid, bpm=60.0, start=0.0, truncate_dur=False):
